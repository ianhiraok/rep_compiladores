...
Symbol *tabela_simbolos = NULL;

Symbol* insere_tabela(const char *txt) {
    Symbol *p = tabela_simbolos;
    while (p != NULL) {
        if (strcmp(p->lexema, txt) == 0) {
            return p;
        }
        p = p->prox;
    }
    Symbol *novo = (Symbol*) malloc(sizeof(Symbol));
    novo->lexema = strdup(txt);
...
typedef struct {
    Symbol *symbol;    /* lexema para ID, INTEGERCONST, CHARCONST, STRINGCONST */
...
static void converter_escapes(char *str) {
    int i = 0, j = 0;
    while (str[i] != '\0') {
        if (str[i] == '\\' && str[i+1] != '\0') {
            i++;
...
static void guarda_lexema(void) {
    microc_yylval.symbol = insere_tabela(yytext);
}
...
'[^'\n]*            {
                        microc_yylval.error_msg = "Constante de caractere invalida ou nao terminada";
                        return UNDEF;
                    }
    ...
    while ((tipo = yylex()) != END_OF_FILE) {
        ...
        if (tipo == ID || tipo == INTEGERCONST || tipo == CHARCONST || tipo == STRINGCONST) {
            printf("Token: tipo = %-13s lexema = '%s'  linha = %d\n",
                   nome_token[tipo], microc_yylval.symbol->lexema, linha_atual);
        } else {
            printf("Token: tipo = %-13s lexema = '%s'  linha = %d\n",
